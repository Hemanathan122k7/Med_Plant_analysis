...
const UIManager = (() => {
    ...
    function displaySearchResults(results, aiRecommendation = null) {
        ...
        setTimeout(() => {
            ...
            if (aiRecommendation) {
                const aiCard = createAIRecommendationCard(aiRecommendation);
                elements.resultsContainer.appendChild(aiCard);
            }
    ...
    function createPlantCard(plant) {
        ...
        const usesTags = plant.uses.map(use => `<span class="use-tag">${use}</span>`).join('');
        const starRating = '⭐'.repeat(Math.floor(plant.rating)) + (plant.rating % 1 >= 0.5 ? '½' : '');
        ...
        card.innerHTML = `
            <div class="plant-header">
                ...
                <div class="plant-badges">
                    <span class="safety-badge ${plant.safety}">${plant.safety === 'safe' ? 'Generally Safe' : 'Use Caution'}</span>
                    <span class="availability-badge ${plant.availability}">${plant.availability.charAt(0).toUpperCase() + plant.availability.slice(1)}</span>
                </div>
            </div>
            <div class="plant-info">
                ...
                <div class="plant-meta">
                    ...
                    <div class="rating">
                        ${starRating} <span class="review-count">(${plant.reviews})</span>
                    </div>
                </div>
    ...
    function createAIRecommendationCard(aiRecommendation) {
        const card = document.createElement('div');
        ...
        card.style.boxShadow = '0 4px 15px rgba(102, 126, 234, 0.3)';
        card.style.animation = 'slideUp 0.6s ease-out';

        // Parse and format AI recommendation
        const formattedText = aiRecommendation.replace(/\n/g, '<br>');
        ...
        card.innerHTML = `
            <div style="display: flex; align-items: center; margin-bottom: 15px;">
                ...
                <div>
                    <h3 style="margin: 0; font-size: 18px; font-weight: 600;">AI-Powered Recommendation</h3>
                    <p style="margin: 5px 0 0; opacity: 0.9; font-size: 14px;">Based on your symptoms description</p>
                </div>
            </div>
